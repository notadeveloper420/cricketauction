...
export default function App() {
  ...
  const goTo = useCallback((s, data) => {
    // Don't re-navigate if already on this screen (prevents subscription double-fires kicking players)
    const hasData = data && Object.keys(data).length > 0
    if (screenRef.current === s && !hasData) return
    screenRef.current = s
    if (hasData) setSession(prev => ({ ...prev, ...data }))
    setScreen(s)
  }, [])
