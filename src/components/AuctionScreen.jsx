...
function runBotBids(room) {
  ...
  room.timerEnd =
    Date.now() + Math.min(botRemaining + 5000, room.timerSecs * 1000);
  room.log.push({
    type: "bid",
    text: `${bot.name} bid ${fmtMoney(newBid, room.league)}`,
    t: Date.now(),
  });
  return room;
}
...
function useCountdown(timerEnd, paused) {
  ...
  useEffect(() => {
    if (!timerEnd) return;
    const tick = () =>
      setSecs(Math.max(0, Math.ceil((timerEnd - Date.now()) / 1000)));
    tick();
      ...
      {currentPool && (
        <div
          style={{
            ...
            fontSize: 11,
            fontWeight: 700,
            background: currentPool.color,
            color: "#0d1117",
            fontFamily: "Barlow Condensed",
            border: `1px solid ${currentPool.color}`,
          }}
        >
          ● {currentPool.label}
        </div>
...
function SoldTransitionPanel({
  ...
  return (
    ...
    >
      ...
      >
        <div
          style={{
            ...
            color: soldResult.buyer ? "var(--green)" : "var(--text-dim)",
            fontWeight: 600,
            marginBottom: 8,
          }}
        >
      ...
      <div
        className="card"
        style={{ borderColor: "var(--border-mid)", position: "relative" }}
      >
        <div
          style={{
            fontSize: 11,
        ...
        {nextPlayer ? (
          <>
            ...
            >
              <span className="badge badge-role" style={{ fontSize: 10 }}>
                {nextPlayer.role}
              </span>
              {nextPlayer.os && (
                <span className="badge badge-os" style={{ fontSize: 10 }}>
                  Overseas
                </span>
              )}
        ...
        >
          <div
            style={{
              ...
              flexShrink: 0,
              background: "var(--bg-elevated)",
              border: `2px solid ${secs > 0 ? "var(--green)" : "var(--text-dim)"}`,
              display: "flex",
              alignItems: "center",
              justifyContent: "center",
              fontFamily: "Barlow Condensed",
      ...
      <div>
        {roles.map((role) => {
          ...
          return (
            ...
            >
              <div
                onClick={() => setOpenPool(isOpen ? null : role)}
                style={{
                  padding: "10px 14px",
                  cursor: "pointer",
                  display: "flex",
      ...
      {pools.map((poolName) => {
        ...
        return (
          <div
            ...
            style={{
              marginBottom: 8,
              border: `1px solid ${currentInPool ? "var(--green)" : "var(--border)"}`,
              borderRadius: "var(--radius-lg)",
              overflow: "hidden",
            }}
          >
            <div
              onClick={() => setOpenPool(isOpen ? null : poolName)}
              style={{
                padding: "10px 14px",
                cursor: "pointer",
                display: "flex",
              ...
              <div>
                <span
                  style={{
                    ...
                    fontWeight: 700,
                    fontSize: 15,
                    color: currentInPool ? "var(--green)" : "var(--text)",
                  }}
                >
                  {poolName}
                </span>
...
// ── Auction log with chat, fade-out bids, highlighted sales ──────────────────
const BID_FADE_MS = 15000; // bids fade after 15s
const SALE_FADE_MS = 120000; // sales stay for 2 min
const CHAT_FADE_MS = 180000; // chats stay for 3 min

function AuctionLog({ log, myName, onSendChat, league }) {
  ...
  function getFadeMs(entry) {
    if (!entry || typeof entry === "string") return BID_FADE_MS;
    if (entry.type === "sale" || entry.type === "unsold") return SALE_FADE_MS;
    if (entry.type === "chat") return CHAT_FADE_MS;
    return BID_FADE_MS;
  }

  function getStyle(entry) {
    if (!entry || typeof entry === "string") return {};
    const age = now - (entry.t || 0);
    const fadeMs = getFadeMs(entry);
    const opacity =
      age > fadeMs
        ? 0
        : age > fadeMs * 0.7
          ? 1 - (age - fadeMs * 0.7) / (fadeMs * 0.3)
          : 1;
    if (opacity <= 0) return null; // signal to hide
    ...
    if (entry.type === "chat")
      return {
        opacity,
        background:
          entry.from === myName ? "var(--blue-bg)" : "var(--bg-elevated)",
        border: `1px solid ${entry.from === myName ? "rgba(88,166,255,0.25)" : "var(--border)"}`,
        borderRadius: 6,
        padding: "6px 10px",
        marginBottom: 3,
      };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
      <div className="log-scroll" style={{ maxHeight: 260 }}>
        {visible.map((entry, i) => {
          ...
          return (
            <div key={i} style={{ ...style, transition: "opacity 0.5s" }}>
              {isChat ? (
                <div>
                  <span
                    style={{
                      ...
                      fontWeight: 600,
                      color:
                        entry.from === myName ? "var(--blue)" : "var(--green)",
                      marginRight: 6,
                    }}
                  >
                    {entry.from}
...
// ── Sold / unsold player viewer ───────────────────────────────────────────────
function SoldViewer({ log, league }) {
  const [filter, setFilter] = useState("all"); // 'all' | 'sold' | 'unsold'

  const entries = (log || []).filter(
    (e) => typeof e !== "string" && (e.type === "sale" || e.type === "unsold"),
  );
  const filtered =
    filter === "sold"
      ? entries.filter((e) => e.type === "sale")
      : filter === "unsold"
        ? entries.filter((e) => e.type === "unsold")
        : entries;
    ...
    <div>
      <div style={{ display: "flex", gap: 6, marginBottom: 10 }}>
        {["all", "sold", "unsold"].map((f) => (
          <button
            key={f}
            className={`timer-opt${filter === f ? " selected" : ""}`}
          ...
          >
            ...
            {f === "all"
              ? `(${entries.length})`
              : f === "sold"
                ? `(${entries.filter((e) => e.type === "sale").length})`
                : `(${entries.filter((e) => e.type === "unsold").length})`}
          </button>
        ))}
      </div>
      {filtered.length === 0 ? (
...
export default function AuctionScreen({ session, goTo }) {
  ...
  async function triggerBotBid() {
    ...
    try {
      ...
      if (updated.currentLeader !== r.currentLeader)
        await setRoom(session.roomCode, updated);
    } catch (e) {}
    processingRef.current = false;
  }
  ...
  async function handleExpiry() {
    ...
    r.timerHandled = true;
    r.soldResult = {
      playerIdx: r.currentIdx,
      playerName: player.name,
      buyer: r.currentLeader,
      price: r.currentBid,
    };
    ...
    if (r.currentLeader) {
      ...
      r.log.push({
        type: "sale",
        text: `✓ ${player.name} → ${r.currentLeader} @ ${fmtMoney(r.currentBid, r.league)}`,
        player: player.name,
        buyer: r.currentLeader,
        price: r.currentBid,
        t: Date.now(),
  ...
  async function placeBid() {
    ...
    if (isFirstBid) {
      ...
      const inc = isHundred
        ...
        : (() => {
            const el = document.getElementById("inc-select");
            return el ? parseInt(el.value) : LEAGUES[r.league].increments[0];
          })();
    ...
    r.log.push({
      type: "bid",
      text: `${myTeamName} bid ${fmtMoney(newBid, r.league)}`,
      t: Date.now(),
    });
    await setRoom(session.roomCode, r);
  }
  ...
  const player = room.players[room.currentIdx];
  const myTeam =
    room.teams.find((t) => t.human === session.myName) ||
    room.teams.find((t) => t.name === session.myTeam);
  const myTeamName = myTeam?.name ?? session.myTeam;
  const myOs = myTeam ? myTeam.squad.filter((p) => p.os).length : 0;
  const total = room.players.length;

  const bidCheck = player
    ? canBid(room, myTeamName, player.os)
    : { ok: false, reason: "" };
  ...
  const bidBlocked =
    !bidCheck.ok || amLeading || !canAfford || room.phase !== "bidding";
    ...
    <div>
        ...
        <div>
          ...
          <div className="progress-bar" style={{ width: 220 }}>
            <div
              ...
              style={{
                width: `${Math.round((room.currentIdx / total) * 100)}%`,
      ...
      {needMore && (
        <div className="banner banner-warn">
          ⚠ Need {l.squadMin - (myTeam?.squad.length ?? 0)} more players,{" "}
          {playersLeft} left
        </div>
      )}
          ...
          >
            ...
            {room.phase === "bidding" && (
              <div style={{ textAlign: "right" }}>
                ...
                <div
                  className={`timer-display${secs <= 3 && !paused ? " warn" : ""}`}
                ...
                {session.isHost && (
                  ...
                  >
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        changeTimerLength(parseInt(e.target.value))
                      }
                    >
                      {[5, 10, 15, 20, 30].map((s) => (
          ...
          <div className="grid3" style={{ marginBottom: 16 }}>
            <div className="stat-card">
              ...
              <div className="stat-value">
                {fmtMoney(player.base, room.league)}
              </div>
            </div>
            <div className="stat-card">
              <div className="stat-label">Current bid</div>
              <div className="stat-value bid-amount">
                {fmtMoney(room.currentBid, room.league)}
              </div>
            </div>
            <div className="stat-card">
            ...
            >
              ...
              >
                {amLeading
                  ? "LEADING"
                  : !bidCheck.ok
                    ? bidCheck.reason?.toUpperCase() || "BLOCKED"
                    : !canAfford
                      ? "INSUFFICIENT"
